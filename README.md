...
- `npm run deposit` and `npm run deposit-cbt` send L1 -> L2 deposits through the Bridgehub. `AMOUNT` is converted from whole tokens to 18 decimals with `cast to-wei` before the transactions are sent.
